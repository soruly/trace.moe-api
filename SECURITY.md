...
Thank you for helping to keep `trace.moe-api` secure. We take security seriously and appreciate your efforts to responsibly disclose vulnerabilities.

This document outlines our policy for reporting security vulnerabilities, supported versions, and the response process.

## Supported Versions

Only the latest major version of `trace.moe-api` is actively supported with security updates. If you find a security issue in an older version, please verify if it also affects the latest version before reporting.
