  ...
  for (let i = 0; i < prodFiles.length; i += BATCH_SIZE) {
    ...
    const valuesSql = batch
      .map((row) => {
        ...
        return `(${id}, ${anilistId}, ${episodeStart}, ${episodeEnd}, ${path}, ${crc32}, ${loaded}, ${created}, ${updated}, ${frameCount}, ${mediaInfo}, ${sceneChanges}, ${colorLayout})`;
      })
      .join(",\n");

    lines.push(
      `INSERT INTO files (id, anilist_id, episode_start, episode_end, path, crc32, loaded, created, updated, frame_count, media_info, scene_changes, color_layout) VALUES\n${valuesSql}\nON CONFLICT (id) DO NOTHING;`,
    );
  }
