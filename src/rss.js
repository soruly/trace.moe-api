...
export default async (req, res) => {
  ...
  const rows =
    await sql`SELECT * FROM files WHERE status='LOADED' ORDER BY updated DESC OFFSET ${Number(offset) >= 0 ? Number(offset) : 0} LIMIT ${Number(limit) > 0 ? (Number(limit) <= 1000 ? Number(limit) : 1000) : 100}`;
