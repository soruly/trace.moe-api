...
describe("/user/reset-key without valid API Key", () => {
  test("No API key", async () => {
    const response = await request(app).post("/user/reset-key");
...
describe("/user/reset-key with valid API Key", () => {
  test("should return new API Key", async () => {
    const response = await request(app)
      .post("/user/reset-key")
    ...
    expect(response.body.key).not.toBe(app.locals.apiKey);
  }, 10000);
});
